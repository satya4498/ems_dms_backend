...
export class SendDocumentRejectionEmailService extends ServiceBase {
  get constraints() {
    return constraints
  }
  ...
  async run() {
    ...
    try {
      let template = await this.context.sequelize.models.emailTemplate.findOne({ where: { eventType: EMAIL_TEMPLATE_EVENT_TYPES.DOCUMENT_REJECTED, isDefault: true } })
      const { result: settings } = await GetSettingsService.execute({ keys: [SETTING_KEYS.APPLICATION_NAME, SETTING_KEYS.ADMIN_END_URL, SETTING_KEYS.DEFAULT_SUPPORT, SETTING_KEYS.LOGO] }, this.context)
      if (!template) {
        template = {
          templateCode: `
            <h3>Your ${this.args.kycLabel} Rejected.</h3>`
        }
      } else {
      ...
      const emailSent = await sendEmail(email, username || email, 'Document Rejected', template.templateCode)

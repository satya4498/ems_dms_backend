...
export class GetGamificationTaskDetailsService extends ServiceBase {
  get constraints () {
    return constraints
  }

  async run () {
    const { search, taskId } = this.args
    const { gamificationTaskGame: gamificationTaskGameModel, gamificationTask: gamificationTaskModel, gamificationTaskCurrency: gamificationTaskCurrencyModel, currency: currencyModel, casinoGame: casinoGameModel } = this.context.sequelize.models
            ...
            include: [
              {
                ...
                as: 'casinoGame',
                attributes: ['id', 'name', 'mobileImageUrl', 'thumbnailUrl']
              }
            ]
          }
        ]

  ...
  async run () {
    ...
    try {
      const user = await this.context.sequelize.models.user.findOne({
        where: { id: userId },
        include: { model: this.context.sequelize.models.userComment },
        transaction
      })
      if (!user) return this.addError('UserDoesNotExistsErrorType')

      let userComment = user.userComment
      if (!userComment) {
        userComment = await this.context.sequelize.models.userComment.create({
          userId,
          title,
          comment,
          commenterId: this.args.adminUserId
        }, { transaction })
      }

...
export class GetPlayerPerformanceCsvReportService extends ServiceBase {
  get constraints () {
    return constraints
  }
    ...
    try {
      ...
      const selectQuery = `
        ...
        SELECT
          ...
          userId AS "USER ID",
          "gcStakedAmount" AS "GC STAKED AMOUNT",
          "gcBetCount" AS "GC BET COUNT",
          "scStakedAmount" AS "SC STAKED AMOUNT",
          "scBetCount" AS "SC BET COUNT",
          "scCasinoWins" AS "SC CASINO WINS",
          "gcCasinoWins" AS "GC CASINO WINS",
          "tournamentEnrollments" AS "TOURNAMENT ENROLLMENTS",
          "tournamentPayouts" AS "TOURNAMENT PAYOUTS",
          "gcPurchases" AS "GC PURCHASES",
          "scRewards" AS "SC REWARDS",
          "redeemAmount" AS "REDEEM AMOUNT",
          "purchaseCount" AS "PURCHASE COUNT",
          "netProfit" AS "NET PROFIT"
      ...
      const reportData = await this.context.sequelize.query(selectQuery, { replacements: replacementsWithPagination, type: this.context.sequelize.QueryTypes.SELECT, logging: console.log })
      const [{ count: totalCount }] = await this.context.sequelize.query(countQuery, { replacements, type: this.context.sequelize.QueryTypes.SELECT, logging: console.log })
